...
export function GoogleButton({ redirectTo, label = 'Continue with Google' }: { redirectTo?: string; label?: string }) {
  ...
  return (
    <Button
      ...
      onClick={async () => {
        ...
        try {
          // This navigates away; the promise only settles on failure.
    ...
    >
      {isRedirecting ? <Spinner className="size-4" /> : <GoogleMark />}
      {label}
    </Button>
  );
}
