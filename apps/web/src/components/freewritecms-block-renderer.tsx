...
import type { Block, CodeContent, HeadingContent, ImageContent, ListContent, QuoteContent } from '@/lib/freewritecms';
...
export function FreeWriteCmsBlockRenderer({ blocks, className }: FreeWriteCmsBlockRendererProps) {
  if (!blocks || blocks.length === 0) {
    return <div className="text-muted-foreground text-center py-8">No content yet</div>;
  }
...
function BlockDisplay({ block }: { block: Block }) {
  switch (block.type) {
    ...
    case 'heading': {
      ...
      return React.createElement(HeadingTag, { className: headingClasses[content.level] }, content.text);
    }
    ...
    case 'image': {
      ...
      return (
        <figure className="my-6">
          ...
          />
          {content.caption && (
            <figcaption className="text-sm text-muted-foreground mt-2 text-center">{content.caption}</figcaption>
          )}
        </figure>
      );
    }
    ...
    case 'code': {
      ...
      return (
        <div className="my-4">
          ...
          {content.language && <div className="text-xs text-muted-foreground mt-1">{content.language}</div>}
        </div>
      );
    }
    ...
    case 'list': {
      ...
      return (
        <ListTag className={cn('ml-6 space-y-1', content.type === 'ordered' ? 'list-decimal' : 'list-disc')}>
    ...
    case 'quote': {
      ...
      return (
        <blockquote className="border-l-4 border-primary pl-4 italic text-muted-foreground my-4">
          {content.text}
          {content.author && <footer className="text-sm mt-2 not-italic">— {content.author}</footer>}
        </blockquote>
      );
    }
